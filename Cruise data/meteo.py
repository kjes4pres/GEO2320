...
def psit_26(z_L):
    ...
    dzet = 0.35 * zet
    dzet[dzet > 50] = 50.0  # stable
    psi = -(
        np.sign((1 + 0.6667 * zet)) * (np.abs((1 + 0.6667 * zet)) ** 1.5)
        + 0.6667 * (zet - 14.28) * exp(-dzet)
        + 8.525
    )
    ...
    psic = 1.5 * log((1.0 + x + x**2) / 3.0) - sqrt(3) * arctan((1 + 2 * x) / sqrt(3))
    psic += 4 * arctan(1.0) / sqrt(3.0)
...
def psiu_26(z_L):
    ...
    psic = 1.5 * log((1.0 + x + x**2) / 3.0) - sqrt(3.0) * arctan(
        (1.0 + 2.0 * x) / sqrt(3.0)
    )
    psic += 4 * arctan(1.0) / sqrt(3.0)
...
def psiu_40(z_L):
    ...
    x = (1.0 - 10.0 * zet[k]) ** 0.3333
    psic = 1.5 * log((1.0 + x + x**2) / 3.0) - sqrt(3.0) * arctan(
        (1.0 + 2.0 * x) / sqrt(3.0)
    )
    psic += 4.0 * arctan(1.0) / sqrt(3.0)
    f = zet[k] ** 2 / (1.0 + zet[k] ** 2)
    psi[k] = (1 - f) * psik + f * psic
    return psi
